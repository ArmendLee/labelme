...
class Shape(object):
    ...
    def addPoint(self, point):
        if self.points and point == self.points[0]:
            ...
            if self._shape_type=='car' and len(self.points)==6:
                self.close()
    ...
    def cal_next_point(self,p1,p2,p3):#計算平行四邊形下一個點
        return QtCore.QPoint(p2.x()-p1.x()+p3.x(),p2.y()-p1.y()+p3.y())
        ...
        if self.points:
            ...
            if self.shape_type == 'rectangle':
                ...
                for i, p in enumerate(self.points):
                    line_path.lineTo(p)
                    self.drawVertex(vrtx_path, i)
            elif self.shape_type == "car":#my edit
                ...
                if len(self.points)>2:
                    ...
                    line_path.lineTo(self.cal_next_point(self.points[0],self.points[1],self.points[2]))
                    line_path.lineTo(self.points[2])
                if len(self.points)>3:
                    line_path.moveTo(self.points[1])
                    line_path.lineTo(self.cal_next_point(self.points[0],self.points[1],self.points[3]))
                ...
                if len(self.points)==6:
                    ...
                    line_path.lineTo(self.cal_next_point(self.points[0],self.points[1],self.points[2]))
                    line_path.lineTo(self.points[2])
                    rectangle = self.getRectFromLine(self.points[4], self.points[5])
                    line_path.addRect(rectangle)
                for i in range(len(self.points)):
                    self.drawVertex(vrtx_path, i)
            else:
